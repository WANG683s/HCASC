...
def train(dataloader, epoch):
    ...
    for i, (img, label) in enumerate(tqdm(dataloader, disable=CONFIG.disable_tqdm)):
        ...
        loss_all += ssl_loss(out["recon"], img)
...
def gan_train(dataloader, epoch):
    ...
    for i, (img, label) in enumerate(tqdm(dataloader, disable=CONFIG.disable_tqdm)):
        ...
        if i % CONFIG.n_critic == 0:
            ...
            g_loss = CONFIG.g_w * kl(fake_validity, torch.ones_like(fake_validity))
...
def ssl_loss(img1, img2, window_size=11, size_average=True):

    channel = img1.size()[1]
    window = torch.Tensor(_ssl_window(window_size, channel)).to(img1.device)
    ...
    C1 = (0.01 ** 2)
    C2 = (0.03 ** 2)
    ssl_map = ((2 * mu1_mu2 + C1) * (2 * sigma12 + C2)) / ((mu1_sq + mu2_sq + C1) * (sigma1_sq + sigma2_sq + C2))

    if size_average:
        return torch.mean((1.0 - ssl_map) / 2)
    else:
        return torch.sum((1.0 - ssl_map) / 2)


def _ssl_window(window_size, channel):
